    ...
    { data?.map((elem) => (
      ...
      <div key={elem.id} className="w-full sm:w-1/2 md:w-1/2 xl:w-1/4 p-4">
        ...
        <div className="relative pb-48 overflow-hidden">
          <img className="absolute inset-0 h-full w-full object-cover" src={elem.data.pimg} alt=""/>
        </div>
        <div className="p-4">
          
          <h2 className="mt-2 mb-2  font-bold">{elem.data.pname}</h2>
          <p className="text-sm">{elem.data.discription}</p>
